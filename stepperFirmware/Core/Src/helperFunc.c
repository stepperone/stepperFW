...
//	  uint16_t x = readADCharrsha(BATTMONchannel);
//	  BATTERY newState = getBatteryState(x);
//
//	  if (newState == FULL){
//		  printf("Battery full\r\n");
//	  }
...
//	xOffsetWhole = a_VarDataTab[3];
//	ee_status |= EE_ReadVariable32bits(4, &a_VarDataTab[4]); // read this part at this address
//	xOffsetDecimal = a_VarDataTab[4];
//	recordings.xOffset = (float)(int32_t)xOffsetWhole +  ((float)(int32_t)xOffsetDecimal / FRACTIONAL_SCALE);
//
//	if (ee_status != EE_OK) {
//		errorState();
//	}
...
//	yOffsetWhole = a_VarDataTab[5];
//	ee_status |= EE_ReadVariable32bits(6, &a_VarDataTab[6]); // read this part at this address
//	yOffsetDecimal = a_VarDataTab[6];
//	recordings.yOffset = (float)(int32_t)yOffsetWhole +  ((float)(int32_t)yOffsetDecimal / FRACTIONAL_SCALE);
//
//	if (ee_status != EE_OK) {
//		errorState();
//	}
...
//	zOffsetWhole = a_VarDataTab[7];
//	ee_status |= EE_ReadVariable32bits(8, &a_VarDataTab[8]); // read this part at this address
//	zOffsetDecimal = a_VarDataTab[8];
//	recordings.zOffset = (float)(int32_t)zOffsetWhole +  ((float)(int32_t)zOffsetDecimal / FRACTIONAL_SCALE);
//
//	if (ee_status != EE_OK) {
//		errorState();
//	}
...
//}
//
//void writeCaliEE(float xOffset, float yOffset, float zOffset)
//{
//	eepromHeader();
//	EE_Status ee_status;
//
//	uint32_t xOffsetWhole = getWhole(xOffset);
//	uint32_t xOffsetDecimal = getDecimal(xOffset);
//	uint32_t yOffsetWhole = getWhole(yOffset);
//	uint32_t yOffsetDecimal = getDecimal(yOffset);
//	uint32_t zOffsetWhole = getWhole(zOffset);
//	uint32_t zOffsetDecimal = getDecimal(zOffset);
//
...
//
//	ee_status = EE_WriteVariable32bits(4, xOffsetDecimal);
//	if ((ee_status & EE_STATUSMASK_CLEANUP) == EE_STATUSMASK_CLEANUP) {ErasingOnGoing = 1;ee_status|= EE_CleanUp_IT();}
//	if ((ee_status & EE_STATUSMASK_ERROR) == EE_STATUSMASK_ERROR) {errorState();}
//
...
//	if ((ee_status & EE_STATUSMASK_ERROR) == EE_STATUSMASK_ERROR) {errorState();}
//
//	ee_status = EE_WriteVariable32bits(6, yOffsetDecimal);
//	if ((ee_status & EE_STATUSMASK_CLEANUP) == EE_STATUSMASK_CLEANUP) {ErasingOnGoing = 1;ee_status|= EE_CleanUp_IT();}
//	if ((ee_status & EE_STATUSMASK_ERROR) == EE_STATUSMASK_ERROR) {errorState();}
//
...
//	if ((ee_status & EE_STATUSMASK_ERROR) == EE_STATUSMASK_ERROR) {errorState();}
//
//	ee_status = EE_WriteVariable32bits(8, zOffsetDecimal);
//	if ((ee_status & EE_STATUSMASK_CLEANUP) == EE_STATUSMASK_CLEANUP) {ErasingOnGoing = 1;ee_status|= EE_CleanUp_IT();}
//	if ((ee_status & EE_STATUSMASK_ERROR) == EE_STATUSMASK_ERROR) {errorState();}
//
//	eepromFooter(ee_status);
